  ...
  addStats(stats) {
    ...
    */
    ...
    Object.entries(stats.kills).forEach(([killerId, killedId]) => {
      const killer = this.stats.players.find((player) => player.id === killerId)
      const killed = this.stats.players.find((player) => player.id === killedId)
      killer.kills++
      killed.killed++
    })
    this.stats.players.forEach((player) => {
      console.log('stats of', player)
    })
    console.log(stats)
